...
AS SELECT * EXCEPT (total) FROM cloud_files("${datasets.path}/orders-raw", "parquet",
                              -- map("cloudFiles.inferColumnTypes", "true"))
                              map(
    "cloudFiles.schemaHints", "order_id STRING, order_timestamp LONG, customer_id STRING, quantity LONG, books ARRAY<STRUCT<book_id: STRING, quantity: LONG, subtotal: LONG>>"
  ))
